# import streamlit as st
# import pandas as pd
# import plotly.express as px

# # Page configuration
# st.set_page_config(page_title="MagicLog Analyzer", layout="wide", page_icon="🔮")

# st.title("🔮 IIS Log Analyzer")
# st.markdown("This dashboard focuses on IIS traffic analysis with specialized Magic xpi/xpa decoding.")
# st.markdown("ℹ️ **Multi-Server Support:** You can upload multiple log files (e.g., from a Load Balancer), and they will be merged into a single view.")

# # --- 1. PARSING FUNCTION ---
# @st.cache_data
# def parse_multiple_files(uploaded_files, log_format):
#     all_dfs = []
#     progress_bar = st.progress(0)
#     total_files = len(uploaded_files)

#     for i, file in enumerate(uploaded_files):
#         progress_bar.progress((i + 1) / total_files)
#         content = file.getvalue().decode("utf-8", errors="ignore")
#         lines = content.splitlines()
#         rows = []
        
#         # FORMAT A: W3C EXTENDED
#         if log_format == "W3C Extended":
#             fields = None
#             data_lines = []
#             for line in lines:
#                 line = line.strip()
#                 if not line: continue
#                 if line.startswith("#"):
#                     if line.lower().startswith("#fields:"):
#                         fields = line.split(":", 1)[1].strip().split()
#                     continue
#                 data_lines.append(line)

#             if fields:
#                 for ln in data_lines:
#                     parts = ln.split()
#                     if len(parts) < len(fields): continue
#                     rows.append(dict(zip(fields, parts)))
#                 temp_df = pd.DataFrame(rows)
#                 if "date" in temp_df.columns and "time" in temp_df.columns:
#                     temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], format="%Y-%m-%d %H:%M:%S", errors="coerce")
#                     all_dfs.append(temp_df)

#         # FORMAT B: IIS CSV
#         elif log_format == "IIS (CSV)":
#             column_map = [
#                 "c-ip", "cs-username", "date", "time", "s-sitename", "s-computername", 
#                 "s-ip", "time-taken", "sc-bytes", "cs-bytes", "sc-status", 
#                 "sc-win32-status", "cs-method", "cs-uri-stem", "cs-uri-query"
#             ]
#             for line in lines:
#                 line = line.strip()
#                 if not line: continue
#                 parts = [p.strip() for p in line.split(',')]
#                 if len(parts) >= len(column_map):
#                     rows.append(dict(zip(column_map, parts[:len(column_map)])))

#             if rows:
#                 temp_df = pd.DataFrame(rows)
#                 if "date" in temp_df.columns and "time" in temp_df.columns:
#                     try:
#                         temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], format="%m/%d/%Y %H:%M:%S", errors="coerce")
#                     except:
#                         temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], errors="coerce")
#                 all_dfs.append(temp_df)

#     progress_bar.empty()

#     if not all_dfs:
#         return None

#     final_df = pd.concat(all_dfs, ignore_index=True)

#     if final_df.empty:
#         return final_df

#     final_df = final_df.dropna(subset=["timestamp"])
#     final_df = final_df.sort_values("timestamp")

#     if "time-taken" in final_df.columns:
#         final_df["time-taken"] = pd.to_numeric(final_df["time-taken"], errors="coerce").fillna(0).astype(int)
    
#     # Pre-calculate Day String
#     final_df["day_str"] = final_df["timestamp"].dt.strftime('%Y-%m-%d')

#     return final_df

# # --- 2. SIDEBAR CONFIGURATION ---
# st.sidebar.header("Configuration")

# log_fmt_option = st.sidebar.radio(
#     "Select Log Format",
#     ("W3C Extended", "IIS (CSV)"),
#     help="Select 'W3C' for space-delimited files. Select 'IIS (CSV)' for comma-delimited."
# )

# uploaded_files = st.sidebar.file_uploader(
#     f"Upload {log_fmt_option} File(s)", 
#     type=["log", "txt", "csv"], 
#     accept_multiple_files=True
# )

# if uploaded_files:
#     with st.spinner(f"Merging and Parsing {len(uploaded_files)} file(s)..."):
#         df_raw = parse_multiple_files(uploaded_files, log_fmt_option)

#     if df_raw is None:
#         st.error("No valid data parsed. Check log format selection.")
#     elif df_raw.empty:
#         st.warning("Data found, but empty after processing.")
#     else:
#         # --- 3. FILTER & TRANSFORM LOGIC ---
#         required_cols = ["cs-uri-stem", "cs-uri-query"]
#         missing_cols = [c for c in required_cols if c not in df_raw.columns]
        
#         if missing_cols:
#             st.error(f"Missing columns: {missing_cols}")
#             st.stop()

#         df_magic = df_raw[df_raw["cs-uri-stem"].str.contains("MgWebRequester.dll", case=False, na=False)].copy()

#         if df_magic.empty:
#             st.warning("No 'MgWebRequester.dll' requests found.")
#             st.stop()

#         df_magic['extracted_app'] = df_magic['cs-uri-query'].str.extract(r"appname=([^&]*)")
#         df_magic['extracted_app'] = df_magic['extracted_app'].fillna("UnknownApp")

#         df_magic['extracted_arg'] = df_magic['cs-uri-query'].str.extract(r"arguments=([^&]*)")
        
#         df_magic['decoded_arg'] = df_magic['extracted_arg'].str.replace('%23', '#', regex=False)
#         df_magic['decoded_arg'] = df_magic['decoded_arg'].fillna("NoArgs")

#         df_magic['full_flow_id'] = df_magic['extracted_app'] + " -> " + df_magic['decoded_arg']

#         # --- 4. DATE FILTER ---
#         st.sidebar.divider()
#         st.sidebar.header("Date Filters")
#         min_date = df_magic["timestamp"].min()
#         max_date = df_magic["timestamp"].max()
        
#         col1, col2 = st.sidebar.columns(2)
#         start_date = col1.date_input("Start Date", min_date.date())
#         end_date = col2.date_input("End Date", max_date.date())

#         mask = (df_magic["timestamp"].dt.date >= start_date) & (df_magic["timestamp"].dt.date <= end_date)
#         df_final = df_magic.loc[mask].copy()

#         # --- 5. DASHBOARD ---
#         total_reqs = len(df_final)
#         unique_flows = df_final['full_flow_id'].nunique()
#         errors = df_final[df_final['sc-status'].astype(str).str.startswith(('4', '5'))]
#         error_count = len(errors)
        
#         k0, k1, k2, k3 = st.columns(4)
#         k0.metric("Files Merged", len(uploaded_files))
#         k1.metric("Total Magic Requests", f"{total_reqs:,}")
#         k2.metric("Unique Flows", f"{unique_flows:,}")
#         k3.metric("Error Responses", f"{error_count:,}", delta_color="inverse")

#         st.divider()

#         # --- SECTION C: HOURLY TRAFFIC ---
#         st.subheader("📈 Hourly Traffic Analysis")

#         df_final["hour"] = df_final["timestamp"].dt.floor("h")
#         hourly_counts = df_final.groupby("hour").size().reset_index(name="requests")
        
#         col_h_chart, col_h_table = st.columns([3, 1])
#         with col_h_chart:
#             fig_hourly = px.bar(hourly_counts, x="hour", y="requests", title="Hourly Load", text_auto=True)
#             st.plotly_chart(fig_hourly, width="stretch")
#             with st.expander("📂 View All Hourly Data"):
#                 hourly_counts["Time"] = hourly_counts["hour"].dt.strftime("%Y-%m-%d %H:00")
#                 st.dataframe(hourly_counts[["Time", "requests"]], hide_index=True, width="stretch")
#         with col_h_table:
#             st.markdown("**🔥 Top 10 Busiest Hours**")
#             top_hours = hourly_counts.sort_values("requests", ascending=False).head(10)
#             top_hours["Time"] = top_hours["hour"].dt.strftime("%Y-%m-%d %H:00")
#             st.dataframe(top_hours[["Time", "requests"]], hide_index=True, width="stretch")


#         # --- SECTION A: TOP COMBINED FLOWS ---
#         st.subheader("🏆 Top Magic Flows (Volume)")
        
#         flow_counts = df_final["full_flow_id"].value_counts().reset_index()
#         flow_counts.columns = ["Flow Identifier", "Count"]
#         top_flows = flow_counts.head(15)

#         col_flow_chart, col_flow_table = st.columns([3, 1])
#         with col_flow_chart:
#             fig_flows = px.bar(top_flows, x="Count", y="Flow Identifier", orientation='h', title="Top 15 Most Executed Flows", text_auto=True, color="Count")
#             fig_flows.update_layout(yaxis=dict(autorange="reversed"))
#             st.plotly_chart(fig_flows, width="stretch")
#         with col_flow_table:
#             st.markdown("**Flow Statistics**")
#             st.dataframe(top_flows, hide_index=True, width="stretch", height=400)
        
#         # --- NEW GRAPH 1: DAILY VOLUME (FACETED AREA CHART) ---
#         st.markdown("#### 📅 Daily Volume by Flow (Trend Lines)")
#         st.caption("Separate area chart for each flow (Top 10), aligned horizontally.")
        
#         top_10_flow_ids = df_final['full_flow_id'].value_counts().head(10).index
#         df_trend_vol = df_final[df_final['full_flow_id'].isin(top_10_flow_ids)]
#         # Group by Flow and Day
#         daily_vol = df_trend_vol.groupby(['full_flow_id', 'day_str']).size().reset_index(name='Requests')
#         daily_vol = daily_vol.sort_values('day_str')

#         # FACETED AREA CHART with PADDING
#         fig_trend_vol = px.area(
#             daily_vol, 
#             x='day_str', 
#             y='Requests', 
#             color='full_flow_id', 
#             facet_col='full_flow_id', 
#             facet_col_wrap=5,
#             facet_col_spacing=0.05,  # Horizontal padding
#             facet_row_spacing=0.15,  # Vertical padding (Crucial)
#             title="Volume Trends per Flow",
#             labels={'day_str': 'Date', 'Requests': 'Count'}
#         )
#         fig_trend_vol.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
#         fig_trend_vol.update_layout(showlegend=False, height=500) # Increased height
#         st.plotly_chart(fig_trend_vol, width="stretch")

#         st.divider()

#         # --- SECTION B: API PERFORMANCE ANALYSIS ---
#         st.subheader("⏱️ API Performance Analysis")
#         st.markdown("Execution time statistics per API Flow (in **Seconds**).")

#         if "time-taken" in df_final.columns:
#             api_perf = df_final.groupby("full_flow_id")["time-taken"].agg(
#                 Min_Time="min", Max_Time="max", Avg_Time="mean", Request_Count="count"
#             ).reset_index()
#             for col in ["Min_Time", "Max_Time", "Avg_Time"]:
#                 api_perf[col] = (api_perf[col] / 1000).round(3)

#             col_perf_chart, col_perf_table = st.columns([2, 1])
#             with col_perf_chart:
#                 slowest_apis = api_perf.sort_values("Avg_Time", ascending=False).head(15)
#                 fig_perf = px.bar(slowest_apis, x="Avg_Time", y="full_flow_id", orientation='h', title="Top 15 Slowest APIs (Avg)", text_auto=True, color="Avg_Time")
#                 fig_perf.update_layout(yaxis=dict(autorange="reversed"))
#                 st.plotly_chart(fig_perf, width="stretch")
#             with col_perf_table:
#                 st.markdown("**Performance Table**")
#                 st.dataframe(api_perf, hide_index=True, width="stretch")

#             # --- NEW GRAPH 2: PERFORMANCE TREND (FACETED AREA CHART) ---
#             st.markdown("#### 📅 Daily Performance by Flow (Trend Lines)")
#             st.caption("Separate area chart for each flow (Top 10 Slowest), aligned horizontally.")

#             slowest_10_ids = api_perf.sort_values("Avg_Time", ascending=False).head(10)['full_flow_id']
#             df_trend_perf = df_final[df_final['full_flow_id'].isin(slowest_10_ids)].copy()
#             daily_perf = df_trend_perf.groupby(['full_flow_id', 'day_str'])['time-taken'].mean().reset_index(name='avg_ms')
#             daily_perf['avg_sec'] = (daily_perf['avg_ms'] / 1000).round(3)
#             daily_perf = daily_perf.sort_values('day_str')

#             # FACETED AREA CHART with PADDING
#             fig_trend_perf = px.area(
#                 daily_perf,
#                 x='day_str',
#                 y='avg_sec',
#                 color='full_flow_id',
#                 facet_col='full_flow_id',
#                 facet_col_wrap=5,
#                 facet_col_spacing=0.05, # Horizontal padding
#                 facet_row_spacing=0.15, # Vertical padding (Crucial)
#                 title="Avg Execution Time Trends per Flow",
#                 labels={'day_str': 'Date', 'avg_sec': 'Seconds'}
#             )
#             fig_trend_perf.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
#             fig_trend_perf.update_layout(showlegend=False, height=500) # Increased height
#             st.plotly_chart(fig_trend_perf, width="stretch")
#         else:
#             st.warning("The 'time-taken' column was not found.")

#         st.divider()

#         # --- SECTION D: PEAK LOAD ---
#         st.subheader("⚡ Peak Load Analysis")
#         col_peak_1, col_peak_2 = st.columns(2)
#         with col_peak_1:
#             peak_sec = df_final["timestamp"].dt.floor("s").value_counts().nlargest(10).reset_index()
#             peak_sec.columns = ["Time", "Requests"]
#             peak_sec["Time Str"] = peak_sec["Time"].dt.strftime("%H:%M:%S")
#             fig_peak = px.bar(peak_sec, x="Requests", y="Time Str", orientation='h', text_auto=True, color="Requests")
#             fig_peak.update_layout(yaxis=dict(autorange="reversed"))
#             st.plotly_chart(fig_peak, width="stretch")
#         with col_peak_2:
#             peak_min = df_final["timestamp"].dt.floor("min").value_counts().nlargest(10).reset_index()
#             peak_min.columns = ["Time", "Requests"]
#             peak_min["Time Str"] = peak_min["Time"].dt.strftime("%H:%M")
#             fig_peak_m = px.bar(peak_min, x="Requests", y="Time Str", orientation='h', text_auto=True, color="Requests")
#             fig_peak_m.update_layout(yaxis=dict(autorange="reversed"))
#             st.plotly_chart(fig_peak_m, width="stretch")

#         # --- NEW GRAPH 3: PEAK LOAD TREND (FACETED AREA CHART) ---
#         st.markdown("#### 📅 Daily Peak Load by Flow (Trend Lines)")
#         st.caption("Separate area chart for each flow (Top 10 Volume), aligned horizontally.")

#         df_rps = df_final.groupby(['timestamp', 'day_str', 'full_flow_id']).size().reset_index(name='rps')
#         df_rps_filtered = df_rps[df_rps['full_flow_id'].isin(top_10_flow_ids)]
#         daily_peak_rps = df_rps_filtered.groupby(['full_flow_id', 'day_str'])['rps'].max().reset_index()
#         daily_peak_rps = daily_peak_rps.sort_values('day_str')

#         # FACETED AREA CHART with PADDING
#         fig_trend_peak = px.area(
#             daily_peak_rps,
#             x='day_str',
#             y='rps',
#             color='full_flow_id',
#             facet_col='full_flow_id',
#             facet_col_wrap=5,
#             facet_col_spacing=0.05, # Horizontal padding
#             facet_row_spacing=0.15, # Vertical padding (Crucial)
#             title="Max RPS Trends per Flow",
#             labels={'day_str': 'Date', 'rps': 'Max RPS'}
#         )
#         fig_trend_peak.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
#         fig_trend_peak.update_layout(showlegend=False, height=500) # Increased height
#         st.plotly_chart(fig_trend_peak, width="stretch")

#         # --- SECTION E: DATA EXPLORER ---
#         st.divider()
#         st.subheader("🔍 Data Explorer")
#         col_filters_1, col_filters_2 = st.columns(2)
#         df_display = df_final.copy()
#         with col_filters_1:
#             all_flows = sorted(df_final['full_flow_id'].unique().tolist())
#             selected_flow = st.selectbox("Filter by Flow:", ["All"] + all_flows)
#             if selected_flow != "All":
#                 df_display = df_display[df_display['full_flow_id'] == selected_flow]
#         with col_filters_2:
#             available_status = sorted(df_final['sc-status'].unique().tolist())
#             selected_status = st.multiselect("Filter by Status Code:", available_status)
#             if selected_status:
#                 df_display = df_display[df_display['sc-status'].isin(selected_status)]

#         st.markdown(f"**Showing {len(df_display)} records**")
#         cols_to_show = ['date', 'time', 'sc-status', 'time-taken', 'extracted_app', 'decoded_arg', 'c-ip', 's-ip']
#         valid_cols = [c for c in cols_to_show if c in df_display.columns]
#         st.dataframe(df_display[valid_cols], width="stretch", height=400)

# else:
#     st.info("Please upload log file(s) to begin analysis.")
import streamlit as st
import pandas as pd
import plotly.express as px
from tracker import track_user

# Page configuration
st.set_page_config(page_title="MagicLog Analyzer", layout="wide", page_icon="🔮")
track_user()
st.title("🔮 IIS Log Analyzer")

# --- NEW: FAQ & DOCUMENTATION SECTION ---
with st.expander("❓ FAQ & Documentation: How does this app work?"):
    st.markdown("""
    ### 🎯 What is this application?
    This is a specialized log analysis tool designed for **IIS Servers** running **Magic xpa/xpi** applications. 
    It parses raw server logs to visualize traffic patterns, performance bottlenecks, and peak load stress points.

    ### ⚙️ How does it work?
    1. **Upload:** You upload `.log` or `.csv` files (supports multi-file upload for Load Balancers).
    2. **Parse & Merge:** The app reads the files, normalizes columns, and merges them into a single timeline.
    3. **Magic Filtering:** It applies a strict filter to keep **only** requests made to `MgWebRequester.dll`.
    4. **Decoding:** It uses Regex to extract the specific `appname` and `arguments` from the complex URL query string (e.g., converting `%23` to `#`).
    
    ### 📊 What do the metrics mean?
    *   **Flow Identifier:** A combination of the Application Name and the Argument (e.g., `App_A -> #ProcessOrder`).
    *   **Time Taken:** The duration (in milliseconds) the server took to process the request. We convert this to seconds for readability.
    *   **RPS (Requests Per Second):** The exact number of hits the server received in a specific 1-second window.
    """)

st.markdown("ℹ️ **Multi-Server Support:** You can upload multiple log files (e.g., from a Load Balancer), and they will be merged into a single view.")

# --- 1. PARSING FUNCTION ---
@st.cache_data
def parse_multiple_files(uploaded_files, log_format):
    all_dfs = []
    progress_bar = st.progress(0)
    total_files = len(uploaded_files)

    for i, file in enumerate(uploaded_files):
        progress_bar.progress((i + 1) / total_files)
        content = file.getvalue().decode("utf-8", errors="ignore")
        lines = content.splitlines()
        rows = []
        
        # FORMAT A: W3C EXTENDED
        if log_format == "W3C Extended":
            fields = None
            data_lines = []
            for line in lines:
                line = line.strip()
                if not line: continue
                if line.startswith("#"):
                    if line.lower().startswith("#fields:"):
                        fields = line.split(":", 1)[1].strip().split()
                    continue
                data_lines.append(line)

            if fields:
                for ln in data_lines:
                    parts = ln.split()
                    if len(parts) < len(fields): continue
                    rows.append(dict(zip(fields, parts)))
                temp_df = pd.DataFrame(rows)
                if "date" in temp_df.columns and "time" in temp_df.columns:
                    temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], format="%Y-%m-%d %H:%M:%S", errors="coerce")
                    all_dfs.append(temp_df)

        # FORMAT B: IIS CSV
        elif log_format == "IIS (CSV)":
            column_map = [
                "c-ip", "cs-username", "date", "time", "s-sitename", "s-computername", 
                "s-ip", "time-taken", "sc-bytes", "cs-bytes", "sc-status", 
                "sc-win32-status", "cs-method", "cs-uri-stem", "cs-uri-query"
            ]
            for line in lines:
                line = line.strip()
                if not line: continue
                parts = [p.strip() for p in line.split(',')]
                if len(parts) >= len(column_map):
                    rows.append(dict(zip(column_map, parts[:len(column_map)])))

            if rows:
                temp_df = pd.DataFrame(rows)
                if "date" in temp_df.columns and "time" in temp_df.columns:
                    try:
                        temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], format="%m/%d/%Y %H:%M:%S", errors="coerce")
                    except:
                        temp_df["timestamp"] = pd.to_datetime(temp_df["date"] + " " + temp_df["time"], errors="coerce")
                all_dfs.append(temp_df)

    progress_bar.empty()

    if not all_dfs:
        return None

    final_df = pd.concat(all_dfs, ignore_index=True)

    if final_df.empty:
        return final_df

    final_df = final_df.dropna(subset=["timestamp"])
    final_df = final_df.sort_values("timestamp")

    if "time-taken" in final_df.columns:
        final_df["time-taken"] = pd.to_numeric(final_df["time-taken"], errors="coerce").fillna(0).astype(int)
    
    # Pre-calculate Day String
    final_df["day_str"] = final_df["timestamp"].dt.strftime('%Y-%m-%d')

    return final_df

# --- 2. SIDEBAR CONFIGURATION ---
st.sidebar.header("Configuration")

log_fmt_option = st.sidebar.radio(
    "Select Log Format",
    ("W3C Extended", "IIS (CSV)"),
    help="Select 'W3C' for space-delimited files. Select 'IIS (CSV)' for comma-delimited."
)

uploaded_files = st.sidebar.file_uploader(
    f"Upload {log_fmt_option} File(s)", 
    type=["log", "txt", "csv"], 
    accept_multiple_files=True
)

if uploaded_files:
    with st.spinner(f"Merging and Parsing {len(uploaded_files)} file(s)..."):
        df_raw = parse_multiple_files(uploaded_files, log_fmt_option)

    if df_raw is None:
        st.error("No valid data parsed. Check log format selection.")
    elif df_raw.empty:
        st.warning("Data found, but empty after processing.")
    else:
        # --- 3. FILTER & TRANSFORM LOGIC ---
        required_cols = ["cs-uri-stem", "cs-uri-query"]
        missing_cols = [c for c in required_cols if c not in df_raw.columns]
        
        if missing_cols:
            st.error(f"Missing columns: {missing_cols}")
            st.stop()

        df_magic = df_raw[df_raw["cs-uri-stem"].str.contains("MgWebRequester.dll", case=False, na=False)].copy()

        if df_magic.empty:
            st.warning("No 'MgWebRequester.dll' requests found.")
            st.stop()

        df_magic['extracted_app'] = df_magic['cs-uri-query'].str.extract(r"appname=([^&]*)")
        df_magic['extracted_app'] = df_magic['extracted_app'].fillna("UnknownApp")

        df_magic['extracted_arg'] = df_magic['cs-uri-query'].str.extract(r"arguments=([^&]*)")
        
        df_magic['decoded_arg'] = df_magic['extracted_arg'].str.replace('%23', '#', regex=False)
        df_magic['decoded_arg'] = df_magic['decoded_arg'].fillna("NoArgs")

        df_magic['full_flow_id'] = df_magic['extracted_app'] + " -> " + df_magic['decoded_arg']

        # --- 4. DATE FILTER ---
        st.sidebar.divider()
        st.sidebar.header("Date Filters")
        min_date = df_magic["timestamp"].min()
        max_date = df_magic["timestamp"].max()
        
        col1, col2 = st.sidebar.columns(2)
        start_date = col1.date_input("Start Date", min_date.date())
        end_date = col2.date_input("End Date", max_date.date())

        mask = (df_magic["timestamp"].dt.date >= start_date) & (df_magic["timestamp"].dt.date <= end_date)
        df_final = df_magic.loc[mask].copy()

        # --- 5. DASHBOARD ---
        total_reqs = len(df_final)
        unique_flows = df_final['full_flow_id'].nunique()
        errors = df_final[df_final['sc-status'].astype(str).str.startswith(('4', '5'))]
        error_count = len(errors)
        
        k0, k1, k2, k3 = st.columns(4)
        k0.metric("Files Merged", len(uploaded_files))
        k1.metric("Total Magic Requests", f"{total_reqs:,}")
        k2.metric("Unique Flows", f"{unique_flows:,}")
        k3.metric("Error Responses", f"{error_count:,}", delta_color="inverse")

        st.divider()

        # --- SECTION C: HOURLY TRAFFIC ---
        st.subheader("📈 Hourly Traffic Analysis")
        with st.expander("ℹ️ How is this calculated?"):
            st.markdown("""
            **Purpose:** To see the general load pattern across the selected date range.
            *   **Calculation:** We take every timestamp, round it down to the nearest Hour (e.g., `14:45:22` becomes `14:00:00`), and count the number of rows.
            *   **Top 10 Hours:** Simple sorting to find which hours had the absolute highest number of hits.
            """)

        df_final["hour"] = df_final["timestamp"].dt.floor("h")
        hourly_counts = df_final.groupby("hour").size().reset_index(name="requests")
        
        col_h_chart, col_h_table = st.columns([3, 1])
        with col_h_chart:
            fig_hourly = px.bar(hourly_counts, x="hour", y="requests", title="Hourly Load", text_auto=True)
            st.plotly_chart(fig_hourly, width="stretch")
            with st.expander("📂 View All Hourly Data"):
                hourly_counts["Time"] = hourly_counts["hour"].dt.strftime("%Y-%m-%d %H:00")
                st.dataframe(hourly_counts[["Time", "requests"]], hide_index=True, width="stretch")
        with col_h_table:
            st.markdown("**🔥 Top 10 Busiest Hours**")
            top_hours = hourly_counts.sort_values("requests", ascending=False).head(10)
            top_hours["Time"] = top_hours["hour"].dt.strftime("%Y-%m-%d %H:00")
            st.dataframe(top_hours[["Time", "requests"]], hide_index=True, width="stretch")


        # --- SECTION A: TOP COMBINED FLOWS ---
        st.subheader("🏆 Top Magic Flows (Volume)")
        with st.expander("ℹ️ How is this calculated?"):
            st.markdown("""
            **Purpose:** Identifies which specific API endpoints are being hit the most.
            *   **Extraction:** We look at the `cs-uri-query` part of the log. 
            *   **Regex Logic:** We extract the value after `appname=` and `arguments=`.
            *   **Normalization:** We replace URL encoded characters like `%23` with `#`.
            *   **Ranking:** We count the occurrences of each unique `AppName -> Argument` pair and show the Top 15.
            """)
        
        flow_counts = df_final["full_flow_id"].value_counts().reset_index()
        flow_counts.columns = ["Flow Identifier", "Count"]
        top_flows = flow_counts.head(15)

        col_flow_chart, col_flow_table = st.columns([3, 1])
        with col_flow_chart:
            fig_flows = px.bar(top_flows, x="Count", y="Flow Identifier", orientation='h', title="Top 15 Most Executed Flows", text_auto=True, color="Count")
            fig_flows.update_layout(yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig_flows, width="stretch")
        with col_flow_table:
            st.markdown("**Flow Statistics**")
            st.dataframe(top_flows, hide_index=True, width="stretch", height=400)
        
        # --- NEW GRAPH 1: DAILY VOLUME (FACETED AREA CHART) ---
        st.markdown("#### 📅 Daily Volume by Flow (Trend Lines)")
        
        top_10_flow_ids = df_final['full_flow_id'].value_counts().head(10).index
        df_trend_vol = df_final[df_final['full_flow_id'].isin(top_10_flow_ids)]
        daily_vol = df_trend_vol.groupby(['full_flow_id', 'day_str']).size().reset_index(name='Requests')
        daily_vol = daily_vol.sort_values('day_str')

        # FACETED AREA CHART with PADDING
        fig_trend_vol = px.area(
            daily_vol, 
            x='day_str', 
            y='Requests', 
            color='full_flow_id', 
            facet_col='full_flow_id', 
            facet_col_wrap=5,
            facet_col_spacing=0.05,
            facet_row_spacing=0.15,
            title="Volume Trends per Flow",
            labels={'day_str': 'Date', 'Requests': 'Count'}
        )
        fig_trend_vol.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
        fig_trend_vol.update_layout(showlegend=False, height=500) 
        st.plotly_chart(fig_trend_vol, width="stretch")

        st.divider()

        # --- SECTION B: API PERFORMANCE ANALYSIS ---
        st.subheader("⏱️ API Performance Analysis")
        with st.expander("ℹ️ How is this calculated?"):
            st.markdown("""
            **Purpose:** To find slow-performing APIs.
            *   **Source:** Uses the `time-taken` field from the IIS log (measured in milliseconds).
            *   **Conversion:** We divide by 1000 to show the result in **Seconds**.
            *   **Aggregation:** For every unique Flow ID, we calculate the Average, Minimum, and Maximum time taken.
            *   **Ranking:** The charts show the top 15 flows sorted by **Highest Average Duration**.
            """)

        if "time-taken" in df_final.columns:
            api_perf = df_final.groupby("full_flow_id")["time-taken"].agg(
                Min_Time="min", Max_Time="max", Avg_Time="mean", Request_Count="count"
            ).reset_index()
            for col in ["Min_Time", "Max_Time", "Avg_Time"]:
                api_perf[col] = (api_perf[col] / 1000).round(3)

            col_perf_chart, col_perf_table = st.columns([2, 1])
            with col_perf_chart:
                slowest_apis = api_perf.sort_values("Avg_Time", ascending=False).head(15)
                fig_perf = px.bar(slowest_apis, x="Avg_Time", y="full_flow_id", orientation='h', title="Top 15 Slowest APIs (Avg)", text_auto=True, color="Avg_Time")
                fig_perf.update_layout(yaxis=dict(autorange="reversed"))
                st.plotly_chart(fig_perf, width="stretch")
            with col_perf_table:
                st.markdown("**Performance Table**")
                st.dataframe(api_perf, hide_index=True, width="stretch")

            # --- NEW GRAPH 2: PERFORMANCE TREND (FACETED AREA CHART) ---
            st.markdown("#### 📅 Daily Performance by Flow (Trend Lines)")
            
            slowest_10_ids = api_perf.sort_values("Avg_Time", ascending=False).head(10)['full_flow_id']
            df_trend_perf = df_final[df_final['full_flow_id'].isin(slowest_10_ids)].copy()
            daily_perf = df_trend_perf.groupby(['full_flow_id', 'day_str'])['time-taken'].mean().reset_index(name='avg_ms')
            daily_perf['avg_sec'] = (daily_perf['avg_ms'] / 1000).round(3)
            daily_perf = daily_perf.sort_values('day_str')

            fig_trend_perf = px.area(
                daily_perf,
                x='day_str',
                y='avg_sec',
                color='full_flow_id',
                facet_col='full_flow_id',
                facet_col_wrap=5,
                facet_col_spacing=0.05,
                facet_row_spacing=0.15,
                title="Avg Execution Time Trends per Flow",
                labels={'day_str': 'Date', 'avg_sec': 'Seconds'}
            )
            fig_trend_perf.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
            fig_trend_perf.update_layout(showlegend=False, height=500)
            st.plotly_chart(fig_trend_perf, width="stretch")
        else:
            st.warning("The 'time-taken' column was not found.")

        st.divider()

        # --- SECTION D: PEAK LOAD ---
        st.subheader("⚡ Peak Load Analysis")
        with st.expander("ℹ️ How is this calculated?"):
            st.markdown("""
            **Purpose:** To identify stress points and concurrency issues (DDoS patterns or heavy bursts).
            *   **Max RPS (Requests Per Second):** We group data by every single second (e.g., `10:00:01`, `10:00:02`). The highest count found is the Peak RPS.
            *   **Max RPM (Requests Per Minute):** Similar logic, but grouped by minute.
            *   **Daily Trends:** For each specific flow, we find the single busiest second in that day and plot it to show if bursts are increasing over time.
            """)

        col_peak_1, col_peak_2 = st.columns(2)
        with col_peak_1:
            peak_sec = df_final["timestamp"].dt.floor("s").value_counts().nlargest(10).reset_index()
            peak_sec.columns = ["Time", "Requests"]
            peak_sec["Time Str"] = peak_sec["Time"].dt.strftime("%H:%M:%S")
            fig_peak = px.bar(peak_sec, x="Requests", y="Time Str", orientation='h', text_auto=True, color="Requests")
            fig_peak.update_layout(yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig_peak, width="stretch")
        with col_peak_2:
            peak_min = df_final["timestamp"].dt.floor("min").value_counts().nlargest(10).reset_index()
            peak_min.columns = ["Time", "Requests"]
            peak_min["Time Str"] = peak_min["Time"].dt.strftime("%H:%M")
            fig_peak_m = px.bar(peak_min, x="Requests", y="Time Str", orientation='h', text_auto=True, color="Requests")
            fig_peak_m.update_layout(yaxis=dict(autorange="reversed"))
            st.plotly_chart(fig_peak_m, width="stretch")

        # --- NEW GRAPH 3: PEAK LOAD TREND (FACETED AREA CHART) ---
        st.markdown("#### 📅 Daily Peak Load by Flow (Trend Lines)")
        
        df_rps = df_final.groupby(['timestamp', 'day_str', 'full_flow_id']).size().reset_index(name='rps')
        df_rps_filtered = df_rps[df_rps['full_flow_id'].isin(top_10_flow_ids)]
        daily_peak_rps = df_rps_filtered.groupby(['full_flow_id', 'day_str'])['rps'].max().reset_index()
        daily_peak_rps = daily_peak_rps.sort_values('day_str')

        fig_trend_peak = px.area(
            daily_peak_rps,
            x='day_str',
            y='rps',
            color='full_flow_id',
            facet_col='full_flow_id',
            facet_col_wrap=5,
            facet_col_spacing=0.05,
            facet_row_spacing=0.15,
            title="Max RPS Trends per Flow",
            labels={'day_str': 'Date', 'rps': 'Max RPS'}
        )
        fig_trend_peak.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))
        fig_trend_peak.update_layout(showlegend=False, height=500)
        st.plotly_chart(fig_trend_peak, width="stretch")

        # --- SECTION E: DATA EXPLORER ---
        st.divider()
        st.subheader("🔍 Data Explorer")
        with st.expander("ℹ️ How to use this?"):
            st.markdown("""
            **Purpose:** To drill down into the raw logs for debugging specific issues.
            1.  **Filter by Flow:** Select a specific API flow to see only its records.
            2.  **Filter by Status:** Select `500` or `404` to see error logs only.
            3.  **Columns:** Displays Timestamp, Status, Time Taken, Client IP, and the decoded Flow Arguments.
            """)

        col_filters_1, col_filters_2 = st.columns(2)
        df_display = df_final.copy()
        with col_filters_1:
            all_flows = sorted(df_final['full_flow_id'].unique().tolist())
            selected_flow = st.selectbox("Filter by Flow:", ["All"] + all_flows)
            if selected_flow != "All":
                df_display = df_display[df_display['full_flow_id'] == selected_flow]
        with col_filters_2:
            available_status = sorted(df_final['sc-status'].unique().tolist())
            selected_status = st.multiselect("Filter by Status Code:", available_status)
            if selected_status:
                df_display = df_display[df_display['sc-status'].isin(selected_status)]

        st.markdown(f"**Showing {len(df_display)} records**")
        cols_to_show = ['date', 'time', 'sc-status', 'time-taken', 'extracted_app', 'decoded_arg', 'c-ip', 's-ip']
        valid_cols = [c for c in cols_to_show if c in df_display.columns]
        st.dataframe(df_display[valid_cols], width="stretch", height=400)

else:
    st.info("Please upload log file(s) to begin analysis.")