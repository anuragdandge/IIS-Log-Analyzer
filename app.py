...
if uploaded_files:
    ...
    if df_raw is None:
        ...
        # --- NEW GRAPH 1: DATES vs FLOW (VOLUME) ---
        st.markdown("#### 📅 Daily Volume Trends (Top 10 Flows)")
        ...
        fig_trend_vol = px.bar(
            ...
            y='Requests', 
            color='full_flow_id', 
            title="Daily Request Volume by Flow",
        ...
        if "time-taken" in df_final.columns:
            ...
            # --- NEW GRAPH 2: DATES vs FLOW (PERFORMANCE) ---
            st.markdown("#### 📅 Daily Performance Trends (Top 10 Slowest Flows)")
            ...
            fig_trend_perf = px.bar(
                ...
                y='avg_sec',
                color='full_flow_id',
                title="Daily Average Execution Time by Flow",
                labels={'day_str': 'Date', 'avg_sec': 'Avg Time (s)', 'full_flow_id': 'Flow Name'},
                barmode='group' # Grouped bars to compare speeds side-by-side
            )
        ...
        # --- NEW GRAPH 3: DATES vs FLOW (PEAK LOAD) ---
        st.markdown("#### 📅 Daily Peak Load Trends (Max RPS per Flow)")
        # Calculate RPS per flow, per second
        # Note: We group by timestamp(1s) + flow + date, then count
        ...
        fig_trend_peak = px.bar(
            ...
            y='rps',
            color='full_flow_id',
            title="Daily Max Requests Per Second (RPS) by Flow",
